...
grafico1<-ggplot(data= ds, mapping= aes(x = MinTemp, y=MaxTemp)) + geom_point()
grafico1

grafico2<-ggplot(data = ds) +
  geom_point(mapping = aes(x = MinTemp, y=MaxTemp, colour=RainTomorrow)) + geom_point()
grafico2

grafico3<-grafico2 + ggtitle('Relación entre temperaturas máxima y mínima y probabilidad de lluvia')
grafico3

ggsave('figs/Ejercicio1.pdf') #coge el ultimo grafico hecho
```
...
ej2_a<-ggplot(data=ds, mapping = aes(x=Temp9am, y=Pressure9am))+geom_point()
ej2_a

ej2_b<-ggplot(data=ds, mapping = aes(x=Temp9am, y=Pressure9am, shape=RainToday)) + geom_point()
ej2_b

ej2_c<-ggplot(data=ds, mapping = aes(x=Temp9am, y=Pressure9am, colour=RainToday)) + geom_point()ej2_a
ej2_c

ej2_d<-ej2_c+labs(title='Probabilidad de lluvia segun condiciones atmosfericas', x='Temperatura', y='Presión atmosferica')
ej2_d
...
iris.grafico2<-ggplot(data=iris.tidy, aes(x=Species, y=Value, colour=Part))+geom_point()
iris.grafico2

iris.grafico3<-ggplot(data=iris.tidy, aes(x=Species, y=Value, colour=Part))+geom_point()+ facet_grid(Measure~.)
iris.grafico3

iris.grafico4<-ggplot(data=iris.tidy, aes(x=Species, y=Value, colour=Part))+geom_jitter()+facet_grid(Measure~.)
iris.grafico4
...
str(DatosEjer4)
ej4_a<-ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()
ej4_a

ej4_b<-ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()+geom_line()
ej4_b

ej4_c<-ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()+geom_line(aes(linetype=Fabricante))
ej4_c

ej4_d<-ej4_c + ggtitle('Ventas mensuales de diferentes compañías tecnológicas')
ej4_d
...
str(Experimento)
ej5_a<-ggplot(data=Experimento, aes(x=Dosis, y=Altura, linetype=Complemento, shape=Complemento)) + geom_point() + geom_line()
ej5_a

ej5_b<- ej5_a + geom_errorbar(aes(ymin=Altura - Error_Altura, ymax=Altura+Error_Altura), width=0.1)
ej5_b

ej5_c<-ggplot(data=Experimento, aes(x=Dosis, y=Altura)) + geom_point(aes(colour=Complemento, shape=Complemento)) + geom_line(aes( colour=Complemento, linetype=Complemento)) + geom_errorbar(aes(ymin=Altura - Error_Altura, ymax=Altura+Error_Altura, colour=Complemento), width=0.1) + scale_color_brewer(palette = 'Reds')
ej5_c
...
?tseries::ice.river
ej6_a<-ggplot(data=ice.river.df, aes(x=time)) +geom_line(aes(y=flow.vat), colour='red')+geom_line(aes(y=flow.jok), colour='blue')
ej6_a

ej6_b<-ej6_a + labs(title='Caudal de los ríos Vatnsdalsa (azul) y Jokulsa (rojo) durante 3 años', x='Tiempo', y =TeX('$Caudal(m^3/s)$'))
ej6_b
ggsave('figs/Ejercicio6.pdf')

#TENER EN CUENTA: En el gráfico NO SE IDENTIFICA QUE RIO ES CUAL. Esto solo se hace si en el dataframe tenemos todas las x en la misma columna (necesitariamos una columna del flujo, que incluiria ambos rios) y que luego otra columna identifique el rio que es (una nueva columna que establezca que rio es). De esta manera graficariamos x=flujo y colour=rio (como hicimos en ejercicios anteriores).
```
...
ej7_a<-ggplot(data=ds, aes(x=Year, y=MinTemp)) + geom_boxplot()
ej7_a

ej7_b<-ggplot(data=ds, aes(x=Year, y=MaxTemp)) + geom_violin(aes(fill=Year)) + labs(title='Distribución de temperaturas máximas por año', x='Año', y='Temperatura (ºC)')
ej7_b

ej7_d<-ggplot(data=ds, aes(x=Year, y=MaxTemp)) + geom_violin(aes(fill=Year)) + labs(title='Distribución de temperaturas máximas por año', x='Año', y='Temperatura (ºC)') + theme(legend.position='none', axis.text.x=element_text(angle=45))
ej7_d
...
dc<-read.table(url, head=T)
df<-data.frame(dc)

ej8_a<-ggplot(data=df, aes(x=HEIGHT)) + geom_histogram()
ej8_a

ej8_b<- ej8_a + labs(title="Histograma de la variable Altura", x="Altura (cm)", y= "Frecuencia")
ej8_b

ej8_c<-ggplot(data=df, aes(x=HEIGHT)) + geom_histogram(breaks= seq(155, 195, by=2))
ej8_c

ej8_d<-ggplot(data=df, aes(x=HEIGHT)) + geom_histogram(breaks= seq(155, 195, by=2), col="red",fill="blue",alpha = .5) 
ej8_d

ej8_e<-ggplot(data=df, aes(x=HEIGHT)) + geom_histogram(aes(y= after_stat(density)), breaks= seq(155, 195, by=2), col="red",fill="blue",alpha = .5)  +
  geom_density(col="green")
ej8_e
  
ej8_f<- ej8_e+facet_grid(. ~ MORT)
ej8_f
```
